...
- [~] **Agregar selector de status al create form** — ya hecho: `app/(dashboard)/jobs/new/page.tsx:738` expone `<select>` con `JOB_STATUS_SELECTABLE` (OPEN/ACTIVE/ON_HOLD/FILLED/CANCELLED/LOST). DRAFT no está incluido porque depende de "Repensar el set de statuses" (decisión de producto pendiente). (Auditoría 5 jun)
...
- [~] **"Firms engaged" widget interactivo** — ya hecho: en `app/client-portal/dashboard/page.tsx:711` el item del Hiring Progress tiene `onClick: openFirmsDrawer` cuando hay firmas, y el stat card "Active Recruiters" (línea 217-225) abre el mismo drawer. (Auditoría 5 jun)
...
- [~] **Invitar member: chequear si email ya es contacto del cliente** — ya existía en `my-team/page.tsx` y `settings/page.tsx`: debounced lookup a `/api/client-portal/contact-lookup`, pre-fill de name/title + banner azul "X is already a contact on file. We pre-filled their info. Submitting will give them portal access too." Agregado también en `dashboard/page.tsx` para que el form de invite del Quick CTA tenga el mismo flow. (Auditoría 5 jun + este PR)
