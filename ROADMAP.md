...
- [ ] **Reemplazar "Engagements" por "Recruiting Firms"** — rename a nivel UI en el client portal (menú, heading, copy). Mantener `Engagement` en el modelo de DB. Pendiente: `app/client-portal/layout.tsx:97`, `app/client-portal/engagements/page.tsx`, `app/client-portal/engagements/[firmId]/page.tsx`.
- [ ] **Unificar diseño del chat en ambos portales** — auditoría 5 jun: estructura está alineada (tabs INTERNAL/CLIENT_VISIBLE con mismo color scheme, bubbles con avatar+autor+timestamp 24h, day separators, mention picker, multiline + Send). Falta verificación visual lado-a-lado para confirmar paddings/radios/font-size exactos.
- [ ] **Cancelar un job → crear otro hereda info del cancelado** — "Duplicate from cancelled" como opción al crear: arranca con título, salary, location, JD, terms del job cancelado pre-llenos y editables. Hoy `app/(dashboard)/jobs/new/page.tsx` no tiene query param `?fromJobId=` ni picker de cancelados.
- [ ] **Colapsar contactos de una empresa con un click** — en el detalle del cliente la lista de contactos debería tener un toggle expand/collapse de un solo click, no scroll infinito. Hoy `app/(dashboard)/clients/[id]/page.tsx:527-650` renderiza todos los contactos siempre, sin state de colapso.
...
- [ ] **Orden Start/End date + MRR de pérdida en placement** — parcial. En `components/placements/placement-dialog.tsx` el modo edit OS muestra End date checkbox (line 1379) ANTES que Starting date (line 1399); hay que invertir. Además falta inline text que muestre "Losing $X/month × Y meses restantes" al activar el endDate de un OS.
