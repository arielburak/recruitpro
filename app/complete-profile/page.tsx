...
// Welcome step for OAuth signups — Google profile gives us a name
// (sometimes) but never a job title. Manual signup + invite-accept
...
export default function CompleteProfilePage() {
  ...
  useEffect(() => {
    ...
    if ((session?.user as any)?.isClientUser) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!cleanName || !cleanTitle) {
      setError("Both name and job title are required.");
      return;
    }
    setLoading(true);
    try {
      ...
      // Push the new name into the JWT so the header reflects it
      // immediately without forcing a re-login. needsProfileCompletion
      // recomputes server-side on the next session() call.
